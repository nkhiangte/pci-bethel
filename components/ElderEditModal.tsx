...
interface ElderEditModalProps {
  elder: Partial<Staff>;
  onClose: () => void;
  onSave: (elder: Staff) => Promise<void>;
...
const ElderEditModal: React.FC<ElderEditModalProps> = ({ elder, onClose, onSave, onDelete, isLoading, showDeleteConfirm, setShowDeleteConfirm }) => {
  const [formData, setFormData] = useState<Partial<Staff>>(elder);
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col animate-in zoom-in-90 duration-200">
        <div className="p-6 border-b flex justify-between items-center bg-church-50 rounded-t-xl">
          <h3 className="text-xl font-bold text-church-900">{elder.id ? `Edit ${elder.name}` : 'Add New Elder'}</h3>
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="A brief description or role of the elder."
            />
          </div>
          {elder.id && (
            <div className="pt-4 border-t border-slate-100">
              ...
              >
                <Trash2 size={16} className="mr-2" /> Delete Elder
              </button>
            </div>
          )}
        </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-90 duration-200">
            ...
            <p className="text-slate-600 mb-6">Are you sure you want to delete this elder? This action cannot be undone.</p>
            <div className="flex justify-end space-x-3">
              ...
              <button onClick={() => { if (elder.id) onDelete(elder.id); setShowDeleteConfirm(null); }} disabled={isLoading} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition shadow-sm disabled:opacity-50">Delete Elder</button>
            </div>
          </div>
        </div>
      )}
