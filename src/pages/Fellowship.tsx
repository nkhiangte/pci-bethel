                      ...
                      <div className="flex space-x-1 overflow-x-auto no-scrollbar">
                          {kpvmNavLinks.map(link => (
                              ...
                              >
                                  ...
                                  {link.id === 'home' ? 'Home' : link.id === 'buhfaitham' ? 'Buhfaitham' : 'Kristian Chhungkua'}
                              </button>
                          ))}
                      </div>
                  </div>
                  ...
                  {kpvmActiveTab === 'home' && (
                      <div className="space-y-8 animate-in fade-in zoom-in duration-300">
                          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-8">
                              <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center"><Users size={20} className="mr-2 text-church-600"/> {t.fellowship.leadership}</h3>
                              <div className="grid md:grid-cols-2 gap-6 text-sm text-slate-700">
                                  <div className="bg-slate-50 p-4 rounded-lg">
                                      <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{t.fellowship.leader}</span>
                                      <span className="text-lg font-bold">{fellowship.leader || 'N/A'}</span>
                                  </div>
                                  <div className="bg-slate-50 p-4 rounded-lg">
                                      <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{t.fellowship.weeklyService}</span>
    ...
    <div className="bg-slate-50 min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
             <div className="bg-white p-8 rounded-xl shadow-sm">
                 ...
                 <p className="text-slate-600 leading-relaxed mb-6 whitespace-pre-line">{fellowship.description}</p>
                 <div className="grid md:grid-cols-2 gap-6">
                     <div className="p-4 border rounded-lg">
                         <h3 className="font-bold text-slate-700">{t.fellowship.leader}</h3>
                         <p>{fellowship.leader}</p>
                     </div>
                     <div className="p-4 border rounded-lg">
                         <h3 className="font-bold text-slate-700">{t.fellowship.weeklyService}</h3>
                ...
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                    <form onSubmit={handleSave}>
                        <div className="p-6 border-b flex justify-between items-center bg-slate-50">
                            <h3 className="text-xl font-bold text-slate-800">{t.fellowship.edit}</h3>
                            <button type="button" onClick={() => setIsEditing(false)} className="text-slate-400 hover:text-slate-600"><X size={24} /></button>
                        </div>
                        <div className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-1">{t.fellowship.name}</label>
                                <input 
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-1">{t.fellowship.description}</label>
                                <textarea 
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-1">{t.fellowship.logo}</label>
                                <div className="flex items-center gap-4">
                                    <div className="w-16 h-16 rounded border overflow-hidden bg-slate-100 flex-shrink-0 relative group">
                                        <img 
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-1">{t.fellowship.leader}</label>
                                    <input 
                                        className="w-full border p-2 rounded" 
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-1">{t.fellowship.weeklyService}</label>
                                    <input 
                                        className="w-full border p-2 rounded" 
                        ...
                        <div className="p-6 border-t bg-slate-50 flex justify-end space-x-3">
                            <button type="button" onClick={() => setIsEditing(false)} className="px-5 py-2.5 border border-slate-300 text-slate-700 rounded-lg hover:bg-white font-medium transition">{t.fellowship.cancel}</button>
                            <button type="submit" disabled={isSaving} className="px-5 py-2.5 bg-church-600 text-white rounded-lg hover:bg-church-700 font-medium flex items-center shadow-md transition disabled:opacity-50">
                                {isSaving ? <Loader className="animate-spin w-4 h-4 mr-2" /> : <Save size={18} className="mr-2" />} {t.fellowship.save}
                            </button>
                        </div>
                    </form>
                </div>
