...
const KtpGallery: React.FC = () => {
  const { isAdmin } = useAuth();
  const { t } = useLanguage();
  const location = useLocation();
  const navigate = useNavigate();
  
  // Fellowship KTP constant category
  const currentCategory = 'Kristian Ṭhalai Pawl (KTP)';
  const currentCategoryPath = 'ktp';
  
  // Parse URL: /ktp/gallery/:folderId?
    ...
    try {
        if (itemType === 'video') {
            ...
            for (let i = 0; i < selectedFiles.length; i++) {
                ...
                const storagePath = `gallery/ktp/${fileName}`;
  ...
  const handleDeleteFolder = async (id: string, e: React.MouseEvent) => {
    ...
    try {
        ...
        if (currentFolderId === id) navigate(`/ktp/gallery`);
    } catch (error) {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 animate-in fade-in duration-300 overflow-hidden">
      ...
      <div className="bg-slate-50 p-4 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center space-x-2 text-sm text-slate-600">
          <Link to="/ktp/gallery" className="hover:text-church-600 transition">Gallery</Link>
          {parentFolders.map((pf) => (
            <React.Fragment key={pf.id}>
              <ChevronRight size={14} className="text-slate-400" />
              <Link to={`/ktp/gallery/${pf.id}`} className="hover:text-church-600 transition">
                {pf.name}
              </Link>
            </React.Fragment>
          ))}
        ...
        {currentFolderId && (
          <button 
            onClick={() => navigate(currentFolder?.parentId ? `/ktp/gallery/${currentFolder.parentId}` : '/ktp/gallery')}
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-2xl font-bold text-slate-900">
            {currentFolder ? currentFolder.name : 'Picture Gallery'}
          </h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  ...
                  {items.length === 0 && (
                    <div className="col-span-full py-12 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                      ...
                      <p className="text-slate-500">No photos or videos in this folder.</p>
              ...
              <form onSubmit={handleSaveFolder} className="p-6 space-y-4">
                <div>
                  ...
                  <input 
                    required
                    type="text"
                    className="w-full border border-slate-300 rounded-lg p-3 focus:ring-2 focus:ring-church-500 outline-none transition"
                    placeholder="e.g. KTP Conference 2024"
              ...
              <form onSubmit={handleSaveItem} className="p-6 space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    type="text"
                    disabled={uploading}
                    className="w-full border border-slate-300 rounded-lg p-3 focus:ring-2 focus:ring-church-500 outline-none transition disabled:bg-slate-50"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        required
                        type="url"
                        className="w-full border border-slate-300 rounded-lg pl-10 pr-3 py-3 focus:ring-2 focus:ring-church-500 outline-none transition"
